// ─── Firebase Configuration ───────────────────────────────────────────────────
// 1. Acesse https://console.firebase.google.com
// 2. Clique em "Criar projeto" → dê um nome → confirme
// 3. No projeto, clique em "Web" (</>)  → registre o app com um apelido
// 4. Copie os valores do firebaseConfig e cole aqui abaixo
// 5. No menu lateral: Firestore Database → Criar banco de dados → Modo de teste
// 6. Pronto! O projeto vai salvar automaticamente no Firestore.
//
// Enquanto não configurar, os dados ficam salvos no localStorage do navegador.

const FIREBASE_CONFIG = {
  apiKey:            "AIzaSyBAd8FECGIKJEJbd922g-4iWKjzjUx2yPw",
  authDomain:        "projeto-disc-49705.firebaseapp.com",
  projectId:         "projeto-disc-49705",
  storageBucket:     "projeto-disc-49705.firebasestorage.app",
  messagingSenderId: "76614615405",
  appId:             "1:76614615405:web:e031a1466ff74428d65a64",
};
